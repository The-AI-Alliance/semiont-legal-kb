 ...
 * source-section reference) generated by yield.fromContext.
 *
  ...
  try {
    ...
    console.log(
      `Found ${obligationAnnotations.length} unbound Obligation annotation(s). ` +
        `Each will trigger a yield.fromContext call (one LLM completion per obligation).`,
    );
    ...
    for (const a of obligationAnnotations) {
      ...
      if (!newRId) {
        console.warn(`  yield.fromContext gave no resourceId for "${a.text}"`);
        continue;
      }
